...
organise_marker_genes <- function(object, k, p_val, auroc) {
    dat <- rowData(object)[, c(paste0("moSC3_", k, "_markers_clusts"), paste0("moSC3_", k, 
        "_markers_auroc"), paste0("moSC3_", k, "_markers_padj"), "feature_symbol")]
    dat <- dat[dat[, paste0("moSC3_", k, "_markers_padj")] < p_val & !is.na(dat[, paste0("moSC3_", 
        k, "_markers_padj")]), ]
    dat <- dat[dat[, paste0("moSC3_", k, "_markers_auroc")] > auroc, ]
    
    d <- NULL
    
    for (i in sort(unique(dat[, paste0("moSC3_", k, "_markers_clusts")]))) {
        tmp <- dat[dat[, paste0("moSC3_", k, "_markers_clusts")] == i, ]
        tmp <- tmp[order(tmp[, paste0("moSC3_", k, "_markers_auroc")], decreasing = TRUE), ]
        d <- rbind(d, tmp)
    }
...
#' 
#' Reorders the rows of the input data.frame based on the \code{moSC3_k_markers_clusts}
#' column and also keeps only the top 10 genes for each value of \code{moSC3_k_markers_clusts}.
#'
#' @param markers a \code{data.frame} object with the following colnames:
#' \code{moSC3_k_markers_clusts}, \code{moSC3_k_markers_auroc}, \code{moSC3_k_markers_padj}.
#' 
...
organise_de_genes <- function(object, k, p_val) {
    de_genes <- rowData(object)[, paste0("moSC3_", k, "_de_padj")]
...
#' values of \code{k} are provided via \code{ks} argument in the main \code{moSC3} function). 
...
#'
#' @param consensus consensus item of the moSC3 slot of an object of 'SingleCellExperiment' class
